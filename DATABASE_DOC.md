...
## 🏢 13. Dominio de Organizaciones (B2B)

### `company` (Mapeado como `company`)

Representa una organización o empresa en el sistema (el "tenant"). Permite la gestión multi-inquilino.

- **Campos:** `id`, `name`, `slug` (unique), `logoUrl?`, `ruc?`, `website?`, `primaryColor?`, `onboardingStep` (`CompanyOnboardingStatus`), `isActive`.
- **Relaciones:** Tiene un `CompanyPreference`, muchos `CompanyMember`, `CompanyInvitation`, `CompanyNotification` y `CompanyFeatureFlag`.

### `company_preference` (Mapeado como `company_preference`)

Preferencias y objetivos de búsqueda de la empresa.

- **Campos:** `id`, `companyId` (unique), `seekingTypes` (`CompanySeekingType[]`).
- **Constraint:** `companyId` unique — una empresa tiene una sola configuración de preferencias.

### `company_member` (Mapeado como `company_member`)

Tabla pivot que vincula usuarios con empresas, asignando roles y estados.

- **Campos:** `id`, `companyId`, `userId`, `role` (`CompanyRole`), `status` (`MemberStatus`), `invitedBy?`, `joinedAt?`.
- **Constraint:** `[companyId, userId]` unique — un usuario no puede estar dos veces en la misma empresa.
- **Índices:** `[companyId, role]`, `[userId]`.

### `company_invitation` (Mapeado como `company_invitation`)

Gestión de invitaciones para unirse a una empresa vía email.

- **Campos:** `id`, `companyId`, `email`, `role` (`CompanyRole`), `token` (unique), `code`, `codeHash`, `status` (`InvitationStatus`), `invitedBy?`, `expiresAt`, `usedAt?`.
- **Índices:** `[companyId, status]`, `[email]`, `[token]`.

### `company_notification` (Mapeado como `company_notification`)

Notificaciones específicas para miembros de una empresa.

- **Campos:** `id`, `companyId`, `userId`, `type`, `title`, `body?`, `metadata?` (Json), `readAt?`.
- **Índice:** `[companyId, userId, readAt]`.

### `feature`, `role_permission` & `company_feature_flag`

Sistema de control de acceso basado en características (Feature Flags) y permisos por rol.

- **`feature`**: Catálogo global de funcionalidades (ej: "talent_search", "advanced_analytics").
- **`role_permission`**: Define qué roles pueden realizar qué acciones (`canRead`, `canWrite`, `canDelete`) sobre una feature.
- **`company_feature_flag`**: Habilita o deshabilita features específicas para una empresa, permitiendo overrides manuales y expiración (trials).
...
| `UserRole`                | Roles de usuario global: USER, ADMIN.                                                                                                                               |
...
| `CompanyRole`             | Roles dentro de una empresa: ADMIN, ENCARGADO, SUB_ENCARGADO, MIEMBRO.                                                                                              |
| `CompanySeekingType`      | Objetivos de búsqueda de la empresa: TALENT, SCHOLARSHIPS, EMPLOYMENT.                                                                                              |
| `CompanyOnboardingStatus` | Progreso del onboarding B2B: STEP_1, STEP_2, STEP_3, COMPLETED.                                                                                                     |
| `InvitationStatus`        | Estado de una invitación: PENDING, ACCEPTED, EXPIRED, CANCELLED.                                                                                                    |
| `MemberStatus`            | Estado de membresía: ACTIVE, INACTIVE, SUSPENDED.                                                                                                                   |
