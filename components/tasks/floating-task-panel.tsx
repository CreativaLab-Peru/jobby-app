  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        task.status === "IN_PROGRESS" ? "bg-primary/5 border-primary/20" : 
        task.status === "FAILED" ? "bg-destructive/5 border-destructive/20" : "bg-secondary/20 border-transparent"
      )}
    >
      <div className="flex items-start gap-3">
        <div className={cn(
          "p-2 rounded-xl",
          task.status === "SUCCEEDED" ? "bg-green-500/10 text-green-500" :
          task.status === "FAILED" ? "bg-destructive/10 text-destructive" : "bg-primary/10 text-primary"
        )}>
