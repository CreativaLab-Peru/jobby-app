...
import {Calendar, Edit, Eye, MoreVertical, Trash2, Users, Link, User2} from "lucide-react";
...
export function AdminCompanyCard({company}: AdminCompanyCardProps) {
  ...
  const handleGenerateInvitation = () => {
    openInvitationModal(company.id);
  }
  ...
  return (
    <>
      <EntityListItem
        icon={
          <div
            className="flex h-10 w-10 items-center justify-center rounded-2xl bg-primary/10 text-primary text-sm font-bold">
        ...
        metadata={
          <>
            ...
            <div className="flex items-center gap-1.5"><Calendar
              className="h-3.5 w-3.5"/><span>Creada: {formatDate(company.createdAt, "d MMM, yyyy")}</span>
            </div>
          </>
        }
        actions={
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" size="icon"
                      className="h-8 w-8 rounded-full text-muted-foreground"><MoreVertical
            ...
            <DropdownMenuContent align="end" className="w-56 rounded-xl">
              ...
              <DropdownMenuItem
                ...
                className="cursor-pointer font-medium"><Edit
                className="mr-2 h-4 w-4"/> Editar</DropdownMenuItem>
              <DropdownMenuItem
              ...
              <DropdownMenuItem
                ...
                className="cursor-pointer font-medium"><Users className="mr-2 h-4 w-4"/> Gestionar
                invitaciones</DropdownMenuItem>
              <DropdownMenuSeparator/>
              <DropdownMenuItem
                onClick={() => setShowDeleteDialog(true)}
                className="cursor-pointer text-destructive focus:text-destructive font-medium"><Trash2
                className="mr-2 h-4 w-4"/> Eliminar</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        }
        footerActions={<Button variant="accent"
                               onClick={() => router.push(`/admin/companies/${company.id}`)}
                               className="h-9 rounded-lg px-6 text-xs font-bold shadow-sm">Ver
          Empresa</Button>}
      />
      <ConfirmModal isOpen={showDeleteDialog} onOpenChange={setShowDeleteDialog}
                    onConfirm={handleDelete} loading={isDeleting} title="Eliminar empresa"
                    description={<>Se eliminara permanentemente la
                      empresa <strong>{company.name}</strong> y todos sus datos asociados. Esta
