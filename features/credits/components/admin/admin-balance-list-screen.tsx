...
import { Filter, LayoutGrid, List, Search, Wallet, X } from "lucide-react";
...
export function AdminBalanceListScreen({
  ...
  const updateQuery = (updates: Record<string, string | null>) => {
    ...
    const target = queryString ? `/admin/balances?${queryString}` : "/admin/balances";
    startTransition(() => {
      router.push(target);
    });
  };
    ...
    <main className="min-h-[90vh] p-4 md:p-8">
      <div className="mx-auto max-w-7xl">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
          <PageHeader
            title="Balances de Creditos"
            description="Gestiona los balances de creditos de todos los usuarios."
          />
