...
export function AdminCvListScreen({
  ...
  const [showFilters, setShowFilters] = useState(
    Boolean(initialCvType || initialOpportunityType || initialStatus)
  );
  ...
  const hasFilters = Boolean(initialCvType || initialOpportunityType || initialStatus);
  ...
  const updateQuery = (updates: Record<string, string | null>) => {
    ...
    const target = queryString ? `/admin/cv?${queryString}` : "/admin/cv";
    startTransition(() => {
      router.push(target);
    });
  };
              ...
              >
                <div className="relative w-full">
                  ...
                  <Input
                    ...
                    placeholder="Buscar por titulo, nombre o email..."
                    className="pl-9"
                  />
                </div>
                ...
                >
                  ...
                  {hasFilters && (
                    <span className="ml-1 flex h-5 w-5 items-center justify-center rounded-full bg-primary-foreground text-primary text-[10px] font-bold">
                      {[initialCvType, initialOpportunityType, initialStatus].filter(Boolean).length}
                    </span>
                  )}
                </Button>
          ...
          {initialCvs.length > 0 ? (
            <>
              ...
              <div className="flex flex-col gap-3 rounded-2xl border border-border/60 bg-card/50 p-4 md:flex-row md:items-center md:justify-between">
                <div className="text-xs text-muted-foreground">
                  Mostrando {startItem}-{endItem} de {totalCount} CVs
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="secondary"
            ...
            <div className="rounded-2xl border border-dashed border-border/60 bg-secondary/10 dark:bg-secondary/5">
              <EmptyPlaceholder
                ...
                description={
                  hasQuery || hasFilters
                    ? "No se encontraron CVs con esos criterios."
                    : "Crea el primer CV para un usuario."
                }
                action={
                  hasFilters ? (
