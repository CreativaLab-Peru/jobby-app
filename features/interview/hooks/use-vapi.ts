...
export const useVapi = () => {
  ...
  const syncElapsed = (nextElapsed: number) => {
    ...
    if (durationRef.current !== null) {
      setRemainingSeconds(Math.max(durationRef.current - nextElapsed, 0));
    }
  };
  ...
  const finalizeAttempt = useCallback(async (reason: string, useBeacon = false) => {
    ...
    if (reason === "timeout") {
      setStatusMessage("Se agotó el tiempo máximo de la simulación.");
    } else if (reason === "manual") {
      setStatusMessage("La simulación se finalizó manualmente.");
    } else if (reason === "start-error") {
      setStatusMessage("No se pudo iniciar la simulación.");
    } else {
  ...
  useEffect(() => {
    ...
    vapiInstance.on("call-start", () => {
      ...
      clearTimer();
      if (sessionRef.current && durationRef.current !== null) {
        timerRef.current = setInterval(() => {
          ...
          if (limit !== null && nextElapsed >= limit) {
            finishReasonRef.current = "timeout";
            clearTimer();
            void finalizeAttempt("timeout");
            vapiInstance.stop();
          }
        }, 1000);
      }
  ...
  const startCall = useCallback(
    async (opportunityId: string, cvId: string) => {
      if (!vapi) return;
      ...
      try {
        ...
        durationRef.current = context.durationSeconds;
        setDurationSeconds(context.durationSeconds);
        setRemainingSeconds(context.durationSeconds);
        ...
        finishReasonRef.current = "start-error";
        void finalizeAttempt("start-error");
        setIsConnecting(false);
      }
    },
    [finalizeAttempt, vapi],
  );

  const endCall = () => {
    finishReasonRef.current = "manual";
    vapi?.stop();
  };
