...
export function AnalysisLoadingScreen({ tempCvEvaluationId, temporalUserId }: AnalysisLoadingScreenProps) {
    ...
    const initAndPoll = async () => {
      ...
      pollInterval = setInterval(async () => {
        ...
        if (steps.matches === JobStatus.SUCCEEDED) {
          addLog("¡Protocolo completado con éxito!");
  ...
  const steps = [
    { id: "config", label: "Configuración", status: pipeline.config, icon: ShieldCheck, desc: "Migrando datos a tu bóveda segura" },
    { id: "analysis", label: "Auditoría IA", status: pipeline.analysis, icon: Sparkles, desc: "Evaluando score y habilidades" },
    { id: "matches", label: "Match Engine", status: pipeline.matches, icon: Search, desc: "Buscando oportunidades activas" },
  ];
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-background">
      <div className="max-w-xl w-full space-y-10">
        ...
        <div className="text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-xs font-bold uppercase animate-pulse">
        ...
        <div className="grid grid-cols-1 gap-4">
          {steps.map((s) => (
            <div key={s.id} className={cn(
              ...
              <div>
                {s.status === JobStatus.SUCCEEDED ? <CheckCircle2 className="w-6 h-6 text-green-500"/> :
                  s.status === JobStatus.IN_PROGRESS ? <Loader2 className="w-6 h-6 text-primary animate-spin"/> :
        ...
        <div className="bg-black/5 dark:bg-white/5 rounded-3xl p-6 font-mono text-[11px] space-y-2 border border-border/20">
          {logs.map((log, i) => (
