    ...
    return (
      <div className="flex justify-center py-2">
        <button
          ...
          className={cn("p-2 rounded-lg transition-colors", activeRoute ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-secondary")}
        >
    ...
    <>
      <div className="px-3 py-2 relative" ref={dropdownRef}>
        <button
          ...
          className={cn(
            ...
            open ? "border-primary/30 bg-primary/5" : "border-border hover:border-primary/20 hover:bg-secondary/30"
          )}
        >
          <RouteIcon className="w-4 h-4 text-primary shrink-0" />
          <div className="flex-1 min-w-0">
            <p className="text-xs font-bold truncate">{activeRoute?.name ?? "Sin ruta"}</p>
        ...
        {open && (
          <div className="absolute left-3 right-3 top-full mt-1 z-50 rounded-xl border border-border bg-card shadow-lg p-1.5 space-y-0.5 max-h-[280px] overflow-y-auto">
            {routes.map((route) => (
              <RouteItem key={route.id} route={route} isActive={route.id === activeRoute?.id} onSelect={() => handleSelect(route)} />
            ))}
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent>
          ...
          />
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowCreate(false)} disabled={isPending}>Cancelar</Button>
            <Button onClick={handleCreate} disabled={isPending || !newName.trim()}>{isPending ? "Creando..." : "Crear"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
