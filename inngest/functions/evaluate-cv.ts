...
export const evaluateCv = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const { filteredSections, cv, allowedSectionTypes } = await step.run(
      "prepare-data",
      async () => {
    ...
    const evaluation = await step.run("get-or-create-evaluation", async () => {
      if (evaluationId) {
        const existingEvaluation = await prisma.cvEvaluation.findUnique({
    ...
    try {
      // 4. Consumir crédito una sola vez por evaluación iniciada
      creditConsumed = await step.run("consume-analysis-credit", async () => {
        await consumeCredits({
          userId,
          type: CreditBalanceType.AI_ACTIONS,
          amount: 1,
          description: `Análisis de CV ${cvId}`,
        });
      ...
      const aiResult = await step.run("query-ai-evaluator", async () => {
        const prompt = getPromptToEvaluateCv(
          filteredSections,
          cv?.cvType,
          cv?.opportunityType,
          cv?.language,
      ...
      // 6. Persistencia de Resultados (Transacción)
      ...
      await step.run("finalize-process", async () => {
        ...
        await prisma.route.updateMany({
          where: {
            cvId,
            userId,
            status: { in: [RouteStatus.CV_CREATED, RouteStatus.ANALYSIS_PENDING] },
          },
      ...
      await step.run("handle-failure", async () => {
        ...
        await prisma.queueJob.update({
          where: { id: job.id },
          data: { status: JobStatus.FAILED, lastError: error.message },
        });
